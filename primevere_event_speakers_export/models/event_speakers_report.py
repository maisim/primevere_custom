...
class EventSpeakersExportCSV(models.AbstractModel):
    ...
    def _write_track_row(self, writer, track, speaker):
        ...
        if hasattr(track, 'dates') and track.dates:
            ...
            session_time = datetime.time(hour=hour, minute=minute).isoformat(timespec="minutes")
        elif track.date:
            session_date = track.date
            hour = int(track.date_begin_located_hour) if hasattr(track, 'date_begin_located_hour') else 0
            minute = round(60 * (track.date_begin_located_hour - hour)) if hasattr(track, 'date_begin_located_hour') else 0
            session_time = datetime.time(hour=hour, minute=minute).isoformat(timespec="minutes")
        ...
        if track.duration:
            ...
            duration_str = datetime.time(hour=hour, minute=minute).isoformat(timespec="minutes")
        ...
        speaker_bio_long = speaker.curriculum_vitae if speaker and hasattr(speaker, 'curriculum_vitae') and speaker.curriculum_vitae else ""
        ...
        slot_wishes = speaker.slot_wishes if speaker and hasattr(speaker, 'slot_wishes') and speaker.slot_wishes else ""
        invitation_number = speaker.invitation_number if speaker and hasattr(speaker, 'invitation_number') and speaker.invitation_number else ""
        invitation_code = speaker.invitation_code if speaker and hasattr(speaker, 'invitation_code') and speaker.invitation_code else ""
        need_ticket = "Oui" if speaker and hasattr(speaker, 'need_ticket') and speaker.need_ticket else "Non"
        need_parking = "Oui" if speaker and hasattr(speaker, 'need_reserved_car_place') and speaker.need_reserved_car_place else "Non"
        ...
        if speaker:
            ...
            books = self.env['event.track.speaker.book'].search([('speaker_ids', 'in', speaker.id)])
            if books:
                book_names = ", ".join([book.name for book in books if book.name])
                book_editors = ", ".join([book.editor_id.name for book in books if book.editor_id and book.editor_id.name])
                book_providers = ", ".join([book.provider_id.name for book in books if book.provider_id and book.provider_id.name])

        # Meals
        meal_dates = ""
        if speaker and hasattr(speaker, 'meal_date_ids') and speaker.meal_date_ids:
            dates = [meal.date.strftime("%Y-%m-%d") for meal in speaker.meal_date_ids if meal.date]
            meal_dates = ", ".join(dates)

        # Travel booking et expenses
        travel_booking = ""
        travel_expense = ""
        if speaker and hasattr(speaker, 'travel_booking_ids') and speaker.travel_booking_ids:
            travel_booking = ", ".join([f"{booking.name}: {booking.cost}" for booking in speaker.travel_booking_ids])
        if speaker and hasattr(speaker, 'travel_expense_ids') and speaker.travel_expense_ids:
            travel_expense = ", ".join([f"{expense.name}: {expense.cost}" for expense in speaker.travel_expense_ids])
        ...
        if hasattr(track, 'equipment_line_ids') and track.equipment_line_ids:
            ...
            for line in track.equipment_line_ids:
                ...
                equip_text = f"{equip_name} ({equip_qty})" if equip_qty > 1 else equip_name
                
                # Determine if it's electrical equipment (based on name)
                if any(keyword in equip_name.lower() for keyword in ['électr', 'electric', 'prise', 'courant', 'alim']):
