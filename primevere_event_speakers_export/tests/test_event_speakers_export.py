...
class TestEventSpeakersExport(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.format = cls.env["event.track.format"].create({"name": "Conference"})
        ...
        # Create themes
        cls.theme1 = cls.env["event.track.theme"].create({"name": "Technology"})

        cls.theme2 = cls.env["event.track.theme"].create({"name": "Innovation"})

        # Create speakers
        cls.speaker1 = cls.env[
            "event.track.speaker"
        ].create(
            {
        ...
        cls.book = cls.env["event.track.speaker.book"].create(
            {
                "name": "Test Book",
                "editor_id": cls.env["res.partner"]
                .create({"name": "Test Publishing"})
                .id,
    ...
    def test_meal_dates_export(self):
        ...
        meal_date1 = (
            self.env["event.track.speaker.meal.date"]
            .with_context(event_id=self.event.id)
            .create(
                {
    ...
    def test_welcomer_export(self):
        ...
        writer2 = csv.DictWriter(
            output2, fieldnames=report.csv_report_options()["fieldnames"], delimiter=";"
        )
